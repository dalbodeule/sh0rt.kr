...
export default defineEventHandler(async (event) => {
    ...
    if (getMethod(event) === 'GET' && env.ANALYTICS) {
        try {
            ...
            const language = parser.parse(getHeader(event, 'accept-language') ?? '')[0]?.code ?? 'unknown'
            const cf = event.context.cf as Partial<IncomingRequestCfProperties> | undefined
            const rawIp = getHeader(event, 'cf-connecting-ip') ?? getRequestIP(event, { xForwardedFor: true }) ?? ''
