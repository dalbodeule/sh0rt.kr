...
useSeoMeta({
  title: 'sh0rt.kr',
  description: '빠르고 안전한 URL 단축 서비스',
  robots: { all: true },
  ogType: 'website',
  ogSiteName: 'sh0rt.kr',
  ogImage: '/favicon.png',
...
const features = [
  {
    icon: faLink,
    title: '간단한 공유',
    description: '길고 복잡한 주소를 기억하고 공유하기 쉬운 링크로 바꿉니다.',
  },
  {
    icon: faChartColumn,
    title: '링크 분석',
    description: '국가, 브라우저, 언어, 기기별 방문 통계를 한눈에 확인합니다.',
  },
  {
    icon: faQrcode,
    title: 'QR 이미지',
    description: '만든 단축주소를 QR코드 이미지로 바로 내려받을 수 있습니다.',
  },
];
</script>
...
<template>
  <main class="space-y-16 pb-10 pt-4 sm:pt-8">
    ...
    >
      ...
      <div class="relative mx-auto max-w-3xl text-center">
        ...
        <h1 class="text-4xl font-black tracking-tight sm:text-6xl">
          긴 주소는 짧게,<br class="hidden sm:block" />
          공유는 더 간단하게.
        </h1>
        <p class="mx-auto mt-6 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
          안전한 리다이렉트와 방문 통계, QR코드까지 한 번에 제공하는 URL 단축 서비스입니다.
        </p>
        <div class="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <NuxtLink
            ...
            >{{ loggedIn ? '새 링크 만들기' : '무료로 시작하기' }}</NuxtLink
          ><NuxtLink
            v-if="loggedIn"
            to="/manage"
    ...
    >
      <article
        v-for="item in [
          ['가입 사용자', globalStats.users],
          ['생성된 링크', globalStats.urls],
          ['현재 활성 링크', globalStats.activeUrls],
        ]"
        :key="String(item[0])"
        class="text-center"
      >
