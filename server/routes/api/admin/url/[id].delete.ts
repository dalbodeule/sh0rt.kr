...
export default defineEventHandler(async (event) => {
    ...
    if (!Number.isInteger(id) || id <= 0) throw createError({ statusCode: 400, statusMessage: 'Invalid URL id' })

    const db = useDrizzle(event.context.cloudflare.env.DB)
    const target = await db.query.urls.findFirst({ where: eq(urls.id, id), columns: { id: true, uid: true } })
