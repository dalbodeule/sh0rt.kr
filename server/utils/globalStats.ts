...
export async function refreshGlobalStats(binding: unknown): Promise<PublicStats> {
    ...
    await db.insert(globalStats).values({ id: 1, data: JSON.stringify(stats), updated_at: now })
        .onConflictDoUpdate({ target: globalStats.id, set: { data: JSON.stringify(stats), updated_at: now } })
    return stats
}
