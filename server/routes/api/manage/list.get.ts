...
export default defineEventHandler(async (event) => {
    ...
    const [items, totals, stats] = await Promise.all([
        db.select({ id: urls.id, uid: urls.uid, manage_id: urls.manage_id, forward: urls.forward, created_at: urls.created_at, updated_at: urls.updated_at, expires: urls.expires })
            .from(urls).innerJoin(usersToUrls, eq(urls.id, usersToUrls.url)).where(where)
            .orderBy(desc(urls.created_at)).limit(pageSize).offset(offset),
        db.select({ value: count() }).from(urls).innerJoin(usersToUrls, eq(urls.id, usersToUrls.url)).where(where),
        db.select({ total: count(), active: sql<number>`sum(case when ${urls.expires} > unixepoch() then 1 else 0 end)` })
            .from(urls).innerJoin(usersToUrls, eq(urls.id, usersToUrls.url)).where(ownerWhere),
    ])
    const totalOwned = Number(stats[0]?.total ?? 0)
    const active = Number(stats[0]?.active ?? 0)
    return { items, page, pageSize, total: Number(totals[0]?.value ?? 0), stats: { total: totalOwned, active, expired: totalOwned - active } }
})
