...
export default defineEventHandler(async (event: H3Event): Promise<IManageResponse> => {
    ...
    await db.delete(analyticsCache).where(lte(analyticsCache.created_at, new Date(Date.now() - CACHE_TTL)))
    const cachedData = await db.query.analyticsCache.findFirst({ where: eq(analyticsCache.uid, currentShorten.uid) })
    if (cachedData) {
        try {
            const cached = JSON.parse(cachedData.data) as Partial<IAnalyticsResponse>
            if (cached.sourceDomain && cached.sourcePath && cached.requestDomain && cached.requestPath && cached.os) {
    ...
    try {
        ...
        const rawData = await getFromAnalytics(
            `SELECT ${getParams([
                AKeys.ip, AKeys.country, AKeys.region, AKeys.city, AKeys.colo,
                AKeys.latitude, AKeys.longitude, AKeys.browser, AKeys.device,
                AKeys.language, AKeys.os, AKeys.browserVersion, AKeys.sourceDomain,
                AKeys.sourcePath, AKeys.requestDomain, AKeys.requestPath,
