...
const load = async () => {
  ...
  errorMessage.value = '';
  try {
    data.value = await useRequestFetch()<ListResponse>('/api/manage/list', {
      query: {
        page: page.value,
        pageSize: 12,
        q: q.value || undefined,
  ...
  <main class="space-y-7 py-4">
    ...
    <section class="grid gap-4 sm:grid-cols-3">
      ...
      >
        ...
        <p class="mt-1 text-3xl font-bold text-slate-950">{{ item[1] }}</p>
      </article>
    </section>
    <form
    ...
    <div v-else class="grid gap-5 md:grid-cols-2">
      <article
        v-for="url in data.items"
        :key="url.manage_id"
        class="rounded-2xl border p-5 shadow-sm"
        :class="
          isExpired(url.expires) ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'
        "
      >
    ...
    <nav v-if="data.total" class="flex items-center justify-center gap-3">
      <button
        :disabled="page <= 1"
